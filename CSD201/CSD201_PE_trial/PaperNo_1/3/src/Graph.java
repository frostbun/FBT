...
class Graph {
    ...
    void depth2(boolean[] visited, int k, RandomAccessFile f) throws Exception {
        f.writeBytes(String.format(" %c(%d)", v[k], k));
        visited[k] = true;
        for (int i=0; i<n; ++i) {
            if (!visited[i] && a[k][i] > 0) {
                depth2(visited, i, f);
...
//===========================================================================
    ...
    void f2() throws Exception {
       ...
       Your task is to insert statements here, just after this comment,
       to complete the question in the exam paper.*/
        int[] shortestPath = new int[20];
        int[] trace = new int[20];
        boolean[] visited = new boolean[20];
        for (int i=0; i<n; ++i) {
        ...
        shortestPath[0] = 0;
        for (int i=0; i<n && !visited[4]; ++i) {
            int index = -1;
            for (int j=0; j<n; ++j) {
                if (visited[j]) {
                    continue;
                }
                if (index == -1 || shortestPath[j] < shortestPath[index]) {
                    index = j;
                }
            }
            if (index == -1) {
            ...
            visited[index] = true;
            for (int j=0; j<n; ++j) {
                if (shortestPath[index] + a[index][j] < shortestPath[j]) {
                    shortestPath[j] = shortestPath[index] + a[index][j];
                    trace[j] = i;
                }
            }
        }
        ...
        for (int i = output.size()-1; i>=0; --i) {
            f.writeBytes(" " + v[output.get(i)]);
        }
        f.writeBytes("\r\n");
        for (int i = output.size() - 1; i >= 0; --i) {
            f.writeBytes(" " + shortestPath[output.get(i)]);
        }
        //-------------------------------------------------------------------------------------
        f.writeBytes("\r\n");
        f.close();
